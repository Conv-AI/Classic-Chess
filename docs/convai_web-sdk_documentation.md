...
Dedicated helpers for blendshape formats and queue creation. Full function list is in Section 13.
...
3. Credentials and Environment Setup
Obtain credentials
Create/login to your Convai account.
...
connect() starts room and transport setup.
state.isConnected becomes true when room connection is established.
botReady event indicates the character is ready for interaction.
Messages stream through data events into chatMessages.
Audio/video/screen-share are managed through dedicated control managers.
...
memoryManager	MemoryManager | null	Long-term memory API manager. See Memory API docs.
Methods
...
Memory API Documentation - Complete guide with examples
Memory API Usage Examples - Runnable code examples
Memory Types
...
Stop UI input loops/listeners
Destroy AudioRenderer
Disconnect ConvaiClient
Close custom AudioContext (if created)
